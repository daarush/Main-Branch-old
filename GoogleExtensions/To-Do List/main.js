...
addButton.addEventListener("click", () => {
  if (addText.value != "") {
    ...
    newDeletePart.className = "fa-trash"
    newDeletePart.innerHTML = "Delete"
    newDeletePart.style.fontSize = "14px"
...
container.addEventListener("click", (item) => {
  switch (item.target.className) {
    case "check-part":
      ...
      toggleCheckStorage(item.target.parentNode, item.target.parentNode.querySelector(".name").innerHTML)
      break;

    case "part":
      item.target.classList.toggle("checked")
      toggleCheckStorage(item.target, item.target.querySelector(".name").innerHTML)
      break;
...
//deletion
container.addEventListener("click", (item) => {
  if (item.target.className == "fa-trash") { //fas fa-trash
    var par = item.target.parentNode
